...
void setup() {
  ...
  if (Ethernet.begin(mac) == 0) { //如果DHCP不可用 自行指定IP
  ...
  lcd.print("H"); lcd.print("e"); lcd.print("e");
  lcd.print("l"); lcd.print("l"); lcd.print("o"); lcd.print(" ");
  lcd.print("c"); lcd.print("h"); lcd.print("o"); lcd.print("s"); lcd.print("e");
  lcd.print("b"); lcd.print("u"); lcd.print("s");
...
//----------車站----------------
/*
  linezero(), lineone(), linetwo(), linethr(), linefou(), linefiv()   
  都是一樣的結構和邏輯 只是能選的站數不同
  按鈕的邏輯架構都和選擇路線一樣
*/ 
void linezero() { //160LINE
  ...
  //sw
  switch (busstopA) { //選擇到達站並LCD顯示以及寫入字串
    case 1:
      LCD001();
      // sendtoserver[29]='\0';
      arrStop[0] = '\0';
...
void loop() {
  ...
  //讀取按鈕
  clearlab();
  if (Bup == 0 && Bdw == 0 && Sup == 0 && Sdw == 0) { //如果按鈕沒有輸入
    ...
    int i = 0;
    while (sendtoserver[i] != '\0') { //計算字串長度
      i++;
    }
    Serial.print("lenght=");
    Serial.println(i);

    if (client.connect(server, 3000)) { //向server連線 3000port
      Serial.println("good");
    ...
    if (client.available()) {  //接收server回傳訊息
      char c = client.read();
      Serial.print(c);
    }
  ...
  else if (Bup == 1 || Bdw == 1 || Sup == 1 || Sdw == 1 || rfid.PICC_IsNewCardPresent()) { //如果有按下任一的按鈕
    ...
    // sendtoserver[7]='\0';
    switch (busline) { //根據busline選擇並執行 寫入字串以及LCD顯示路線
      case 0:
